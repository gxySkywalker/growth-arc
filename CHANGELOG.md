...
- 完成云丘垂耳兔：小丘 → 云丘兔 → 风茸旅兔
- 完成余烬古龙：小火牙 → 赤翼龙 → 余烬古龙
- 接入八位伙伴的营地肖像、四方向行走／飞行图与 Pixi 小屋成长尺寸
- 伙伴营地显示当前阶段名与物种名，修正小石獾等阶段标题

### 🎒 出征掉落重构

- 扩展为普通、罕见、稀有、珍稀四级，现有 11 件旅途物品
- 加入河岸圆石、风丘羽毛、古塔残页，以及定向伙伴羁绊物品
- 夜间月银罗盘、星辉玻璃、旅者银铃改为可追踪的远征状态
- 收藏类物品不再误消耗；物品说明、悬浮提示与像素风确认窗口统一

### ✉️ 信件与观测修复

- DeepSeek 配置、连通测试与每日／每周 AI 信件润色统一到同一请求链路
- 修复天文台异步数据到达后偶发图表空白，以及切换标签页的旧请求竞争
