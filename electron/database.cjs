...
class StudyDatabase {
  ...
  useItem(itemId) {
    ...
    if (itemId === 'berry_bread') {
      ...
      const growthPath = stage === 2 && !active.evolution_path ? growthPathForCompanion(active.species_id, now) : active.evolution_path
      const growthCompletedAt = stage === 2 && !active.evolution_path ? now : active.growth_completed_at
      const nickname = automaticGrowthNickname(active, stage, growthPath)
      this.run('UPDATE companions SET nickname = ?, bond_xp = ?, stage = ?, evolution_path = ?, growth_completed_at = ? WHERE id = ?', [nickname, newBond, stage, growthPath, growthCompletedAt, active.id], false)
