    ...
    <section className="hearth-hero">
      ...
      <div className="companion-home">
        ...
        <div className="companion-bubble">
          <strong>{companion?.nickname || '栗子'}</strong>
          <span>{today.focusSeconds ? '上次的远征很不错。要再走一程吗？' : '我听见远方有宝箱在响。'}</span>
        </div>
      </div>
    ...
    <section className="home-columns">
      ...
      <article className="parchment-card treasure-shelf">
        ...
        <div className="loot-grid">
          {inventory.length ? inventory.map((entry) => <div className={`loot-tile ${entry.item.rarity}`} key={entry.item_id} title={entry.item.description}>
            <span className="loot-glyph">{entry.item.rarity === 'rare' ? '✦' : entry.item.icon === 'bread' ? '▰' : '◆'}</span>
