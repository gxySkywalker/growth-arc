...
export function SettingsPage() {
  ...
  useEffect(() => {
    window.growthArc.settings.get().then((data) => {
      setName(String(data.user_name || '学习者')); setModel(String(data.model || 'gpt-5.6-luna')); setAccent(String(data.accent || '#8b9cff')); setHasKey(Boolean(data.hasApiKey))
  ...
  const saveProfile = async () => {
    setBusy(true)
    try {
      await window.growthArc.settings.set({ user_name: name, model, accent })
  ...
  return <div className="page settings-page">
    ...
    <div className="settings-stack">
      ...
      <section className="panel settings-section"><header><div className="setting-icon"><Icon name="brain" /></div><div><h2>OpenAI 学习报告</h2><p>使用 Responses API 生成结构化日评和周报；API 使用独立计费。</p></div><span className={`status-pill ${hasKey ? 'connected' : ''}`}>{hasKey ? '已配置' : '未配置'}</span></header><div className="settings-form ai-settings"><label>模型名称<input value={model} onChange={(event) => setModel(event.target.value)} placeholder="gpt-5.6-luna" /><small>默认选择低成本模型；如不可用，可填写你账户有权使用的模型。</small></label><label>OpenAI API Key<input type="password" autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={hasKey ? '已安全保存；输入新值可替换' : 'sk-…'} /><small>密钥由 Windows DPAPI 加密，不进入数据库、前端状态持久化或日志。</small></label><div className="button-row">{hasKey && <button className="button button-danger-ghost" onClick={clearKey}>删除密钥</button>}<button className="button button-secondary" disabled={busy || !apiKey.trim()} onClick={saveKey}>安全保存密钥</button><button className="button button-primary" disabled={busy || !model.trim()} onClick={saveProfile}>保存模型设置</button></div></div></section>
