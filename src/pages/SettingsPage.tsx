...
export function SettingsPage() {
  ...
  const saveProfile = async () => {
    setBusy(true)
    try {
      await window.growthArc.settings.set({ user_name: name, model, ai_base_url: proxyUrl, api_provider: provider })
  ...
  return <div className="page settings-page">
    ...
    <div className="settings-stack">
      ...
      <section className="panel settings-section"><header><div><h2>✦ 小天使的信</h2><p>小天使每天都会整理你的旅途。不需要做任何设置。</p>{hasKey && <p style={{fontSize:12,color:'var(--muted)',margin:'4px 0 0'}}>信笺钥匙已配置 · 小天使可以亲手写信</p>}</div></header><div className="settings-form"><details style={{fontSize:13,color:'var(--muted)',cursor:'pointer'}}><summary style={{marginBottom:8}}>如果想让远方的信使帮助小天使写下更特别的信……</summary><div className="ai-settings" style={{marginTop:8}}><label>信使<select value={provider} onChange={(event) => { const v = event.target.value; setProvider(v); setConfigDirty(true); if (v === 'deepseek') setModel('deepseek-chat'); else if (v === 'openai') setModel('gpt-5.6-luna') }}><option value="deepseek">DeepSeek</option><option value="openai">OpenAI</option><option value="custom">自定义地址</option></select><small>在 DeepSeek 官网注册后可以获得一把钥匙。费用很低。</small></label>{provider === 'custom' && <label>自定义地址<input value={proxyUrl} onChange={(event) => { setProxyUrl(event.target.value); setConfigDirty(true) }} placeholder="https://your-api.example.com/v1" /></label>}<label>信笺钥匙<input type="password" autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={hasKey ? '已安全保存；输入新值可替换' : '输入钥匙…'} /><small>保存在你的电脑上，使用 Windows 安全加密。</small></label><div className="button-row">{configDirty && <button className="button button-secondary" disabled={busy} onClick={saveProfile}>保存信使设置</button>}{hasKey && <button className="button button-danger-ghost" onClick={clearKey}>移除钥匙</button>}<button className="button button-primary" disabled={busy || !apiKey.trim()} onClick={saveKey}>{hasKey ? '更换钥匙' : '保存钥匙'}</button>{hasKey && <button className="button button-ghost" disabled={busy} onClick={async () => { setBusy(true); try { const r = await window.growthArc.mail.testLetter(); if (r.success) notify('小天使收到了一张试写的信纸。从今天开始，她可以帮你整理特别的信件了。', 'success'); else notify('小天使暂时没有收到回应。\n请检查信笺钥匙是否正确，或者稍后再试。', 'error'); } catch (e: any) { notify('小天使暂时没有收到回应。请稍后再试。', 'error'); } finally { setBusy(false) } }}>请小天使试写一封信</button>}</div></div></details></div></section>
